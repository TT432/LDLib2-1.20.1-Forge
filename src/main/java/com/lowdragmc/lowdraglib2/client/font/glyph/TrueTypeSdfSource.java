...
public class TrueTypeSdfSource implements GlyphSource {
    ...
    public static TrueTypeSdfSource load(ResourceManager resourceManager, TrueTypeGlyphProviderDefinition definition,
        ...
        try (InputStream stream = resourceManager.open(file)) {
            ...
            data.put(bytes).flip();
            synchronized (LDFreeTypeUtil.LIBRARY_LOCK) {
                try (var stack = MemoryStack.stackPush()) {
                    var pointer = stack.mallocPointer(1);
                    LDFreeTypeUtil.assertError(
                            FreeType.FT_New_Memory_Face(LDFreeTypeUtil.getLibrary(), data, 0L, pointer),
                            "Initializing font face");
                    face = FT_Face.create(pointer.get());
                }
                LDFreeTypeUtil.assertError(FreeType.FT_Select_Charmap(face, FreeType.FT_ENCODING_UNICODE),
    ...
    public synchronized GlyphMetrics metrics(int codepoint) {
        ...
        if (LDFreeTypeUtil.checkError(FreeType.FT_Load_Glyph(face, index, LOAD_METRICS), "Loading glyph metrics")) {
            return null;
        }
        var slot = face.glyph();
        if (slot == null) {
        ...
        return GlyphMetrics.of(LDFreeTypeUtil.x(slot.advance()) / oversample);
    }
    ...
    private Rendered render(int glyphIndex, int pixelSize, int loadFlags) {
        ...
        if (LDFreeTypeUtil.checkError(FreeType.FT_Load_Glyph(face, glyphIndex, loadFlags), "Rendering glyph")) {
            return null;
        }
        var slot = face.glyph();
        if (slot == null) {
        ...
        var advance = LDFreeTypeUtil.x(slot.advance());
    ...
    private void setPixelSize(int pixelSize) {
        ...
        LDFreeTypeUtil.assertError(FreeType.FT_Set_Pixel_Sizes(face, pixelSize, pixelSize), "Setting pixel size");
    ...
    public synchronized void close() {
        if (face != null) {
            synchronized (LDFreeTypeUtil.LIBRARY_LOCK) {
                LDFreeTypeUtil.checkError(FreeType.FT_Done_Face(face), "Deleting font face");
            }
            face = null;
        }
        if (fontData != null) {
