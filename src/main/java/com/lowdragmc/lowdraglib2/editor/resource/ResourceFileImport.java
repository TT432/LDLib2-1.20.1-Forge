 ...
 * values are built from a {@link ResourceLocation} — an image for a texture, a json for a model.
 * <p>
...
public final class ResourceFileImport {
    ...
    /**
     * Reduces a file name to the characters a {@link ResourceLocation} path accepts, so a file called
    ...
    public static String sanitizeName(String raw) {
        ...
        for (var character : raw.toLowerCase(Locale.ROOT).toCharArray()) {
            builder.append(character != '/' && ResourceLocation.isAllowedInResourceLocation(character)
                    ? character : '_');
        }
     ...
     */
    @Nullable
    public static ResourceLocation locationOf(File file) {
        try {
    ...
    public static void resolveOrImport(UIElement owner, File file, String assetFolder,
                                       Consumer<ResourceLocation> onResolved, Runnable onCancel) {
        ...
        Dialog.stringEditorDialog("editor.resource.import_destination", suggestion,
                ResourceLocation::isValidPath,
                path -> {
                    if (path == null || !ResourceLocation.isValidPath(path)) {
