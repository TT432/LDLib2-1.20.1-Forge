...
public class HDRColor {
    ...
    private static final Codec<HDRColor> CURRENT_CODEC = RecordCodecBuilder.create(instance -> instance.group(
            LDLibExtraCodecs.VECTOR4F.fieldOf("color").forGetter(HDRColor::toRawVector4f),
     ...
     */
    public static final Codec<HDRColor> CODEC = Codec.either(CURRENT_CODEC, LDLibExtraCodecs.VECTOR4F)
            // DFU 6（MC 1.20.1）无 Codec.withAlternative：either + xmap 同语义——读两种形状，写恒为主形状
            .xmap(
    ...
    private static int round255(float value) {
        // Java 21 的 Math.clamp 在 JDK17 不存在
